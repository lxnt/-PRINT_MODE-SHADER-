...
Either<texture_fullid,texture_ttfid> renderer::screen_to_texid(int x, int y) {
 ...
 if ((x==0) && (y==0))
	  fprintf(stderr, "tile(%d,%d) is %d; fg: 0x%02x%02x%02x bg: 0x%02x%02x%02x graphics:%c used_ch:%c used_bw:%c skip_ch:%c used_gs:%c\n",
			  x, y, ret.texpos,
			 (int)(255*ret.r), (int)(255*ret.g), (int)(255*ret.b),
			 (int)(255*ret.br), (int)(255*ret.bb), (int)(255*ret.bg),
			 was_tep, used_ch, used_bw, skip_ch, used_gs
			 );
